...
const artisans = [
  {
    slug: 'maria-santos', name: 'Maria Santos', specialty: 'Ceramics & Pottery',
    location: 'Oaxaca, Mexico', initials: 'MS', memberSince: 'January 2024',
    bio: [
      ...
      'After studying fine arts in Mexico City, I returned to Oaxaca determined to carry forward the Zapotec ceramic tradition while making it my own.',
      'My work is not mass-produced. A single bowl may take three weeks from raw clay to finished piece.',
    ],
    productCount: 12, reviewCount: 87, rating: '4.9',
    products: [
    ...
    social: { instagram: '#', website: '#', etsy: undefined },
  },
  {
    slug: 'juan-aguirre', name: 'Juan Aguirre', specialty: 'Silversmithing',
    location: 'San José, Costa Rica', initials: 'JA', memberSince: 'March 2024',
    bio: [
      'Every piece of jewelry I create starts with a story — a love letter, a milestone, a memory. I hand-forge each item using traditional silversmithing techniques passed down through three generations.',
      'I trained under my father, who trained under his. The tools on my bench are older than I am.',
      'I work exclusively with recycled silver and ethically sourced stones.',
    ],
    productCount: 8, reviewCount: 54, rating: '4.8',
    products: [
  ...
  {
    slug: 'ana-toledo', name: 'Ana Toledo', specialty: 'Textile Weaving',
    location: 'Guatemala City, Guatemala', initials: 'AT', memberSince: 'June 2024',
    bio: [
      'Weaving is how my family has communicated for generations. The patterns are a language — every color and motif carries meaning that words sometimes can\'t hold.',
      'I use natural dyes exclusively: indigo, cochineal, marigold, and bark tannins gathered from the highlands around my studio.',
      'My mission is to make textiles beautiful enough that people genuinely want to live with them.',
    ],
    productCount: 15, reviewCount: 112, rating: '5.0',
    products: [
    ...
    social: { instagram: '#', website: undefined, etsy: '#' },
  },
];
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: artisan.name, description: `${artisan.specialty} from ${artisan.location}.` };
}
...
export default async function ArtisanProfilePage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" style={{ backgroundColor: colors.background, borderBottom: `0.5px solid ${colors.borderWarm}`, padding: '0.75rem 1rem' }}>
        <ol style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', listStyle: 'none', margin: 0, padding: 0, fontFamily: 'sans-serif', fontSize: '0.85rem', flexWrap: 'wrap' }}>
      ...
      <main id="main-content" style={{ backgroundColor: colors.background, minHeight: '100vh' }}>
        ...
        {/* Profile header */}
        <section aria-labelledby="artisan-name" className="section-padding-warm" style={{ backgroundColor: colors.backgroundWarm }}>
          <div className="profile-header" style={{ maxWidth: '900px', margin: '0 auto' }}>
            <div aria-hidden="true" style={{
              width: '120px', height: '120px', backgroundColor: colors.primary,
              borderRadius: '50%', display: 'flex', alignItems: 'center',
              justifyContent: 'center', color: colors.background,
              fontFamily: 'sans-serif', fontSize: '2.5rem', fontWeight: 700,
              flexShrink: 0, border: `3px solid ${colors.secondary}`,
            }}>
              {artisan.initials}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <dl className="profile-stats" style={{ margin: '0.5rem 0 0' }}>
                {[
        ...
        {/* Biography */}
        <section aria-labelledby="bio-heading" className="section-padding" style={{ maxWidth: '900px', margin: '0 auto' }}>
          <h2 id="bio-heading" style={{ ...typography.h2, marginBottom: '1.5rem' }}>
            About {artisan.name.split(' ')[0]}
          </h2>
          <div style={{ borderLeft: `3px solid ${colors.secondary}`, paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        {/* Products */}
        <section aria-labelledby="products-heading" className="section-padding-warm" style={{ backgroundColor: colors.backgroundWarm }}>
                  ...
                  <article style={{ backgroundColor: colors.backgroundCard, borderRadius: '12px', border: `0.5px solid ${colors.border}`, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', flexGrow: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.25rem' }}>
                        <span style={{ fontFamily: "'Georgia', serif", fontSize: '1.1rem', fontWeight: 500, color: colors.primary }}>{product.price}</span>
                        {product.inStock ? (
        ...
        {/* Commission CTA */}
        <section aria-labelledby="commission-heading" className="section-padding" style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
          <h2 id="commission-heading" style={{ ...typography.h2, marginBottom: '0.75rem' }}>Want something custom?</h2>
